...
module.exports = {
	...
	getDisplay: function(exchangeData) {
		...
		while(sellPrices.size() > 0) {
			...
			stringBook += "SELL" + padding + sellPrice + ",  " + sells.volumes[sellPrice] + "\n";
		}

		while(buyPrices.size() > 0) {
			var buyPrice = buyPrices.pop();
			stringBook += "BUY" + padding + buyPrice + ",  " + buys.volumes[buyPrice] + "\n";
		}
...
function createBinaryHeap(options) {
...
function order(orderType, price, volume, exchangeData) {
	...
	if(trade) {
		var oppBook = (orderType == BUY) ? cloned[SELL] : cloned[BUY];
